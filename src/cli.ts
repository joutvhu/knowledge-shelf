...
function cmdValidate(knowledgeDir: string, args: string[]): void {
  ...
  for (const unitName of toValidate) {
    ...
    // Validate modules
    const modules = manifest.modules as Record<string, { files?: string[]; guide?: string; dependencies?: string[] }> | undefined;
    if (modules) {
...
function scanDocumentsForCli(knowledgeDir: string): CliDocEntry[] {
  ...
  walkDir(knowledgeDir, (filePath) => {
    ...
    docs.push({ name: relPath, title, description, tags, aliases, sections: [], modules: [], type: "standalone", has_manifest: false });
  });
